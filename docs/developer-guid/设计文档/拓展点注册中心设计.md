...
graph TD
    ...
    subgraph app模块[app模块 - 配置层]
        F[BootstrapConfig<br/>扩展注册中心装配]
    end

    subgraph plugin模块[plugin模块 - 扩展实现]
        G[StorageProviderImpl<br/>存储能力实现]
        H[AuthProviderImpl<br/>认证能力实现]
    end
...
1. 定义扩展点接口（继承 ExtensionPoint）

```java
public interface NewCapability extends ExtensionPoint {
    void execute();
}
```
...
```java
@Extension(points = NewCapability.class, priority = 100)
public class NewCapabilityImpl implements NewCapability {
    @Override
    public void execute() {
        // 实现逻辑
    }
...
```java
extensionRegistry.register(NewCapability.class, new NewCapabilityImpl(), 100);
```

4. 使用扩展

```java
Optional<NewCapability> capability = extensionRegistry.get(NewCapability.class);
capability.ifPresent(NewCapability::execute);
```
...
```java
// 注册多个实现
extensionRegistry.register(StorageProvider.class, localStorage, 50);
extensionRegistry.register(StorageProvider.class, s3Storage, 100);
extensionRegistry.register(StorageProvider.class, ossStorage, 75);

// 获取最高优先级实现（返回 s3Storage）
Optional<StorageProvider> provider = extensionRegistry.get(StorageProvider.class);

// 获取所有实现（按优先级降序）
List<StorageProvider> allProviders = extensionRegistry.getAll(StorageProvider.class);
// 顺序: s3Storage(100) → ossStorage(75) → localStorage(50)
```
...
sequenceDiagram
    ...
    Plugin->>Manager: 启动
    Manager->>Plugin: onStart()
    Plugin->>Registry: register(ExtensionPoint, impl)
    Registry-->>Plugin: 注册成功
    Plugin-->>Manager: 启动完成

    Note over Plugin,Point: 插件运行期

    Plugin->>Manager: 停止
    Manager->>Registry: unregisterByPluginId(pluginId)
    Registry->>Registry: 清理该插件所有扩展
    Registry-->>Manager: 清理完成
    Manager->>Plugin: onStop()
```
...
- 插件启动时注册其提供的扩展实现
- 插件停止时自动注销其所有扩展
- 通过 `pluginId` 关联扩展与插件，支持插件级生命周期管理
