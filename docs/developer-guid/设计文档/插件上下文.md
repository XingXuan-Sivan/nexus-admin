...
| config() | 访问配置管理器，读写配置 |
| services() | 获取平台服务注册中心 |
| service(type) | 获取指定类型的服务实例（便捷方法） |

------

## 4. 平台服务注册中心

### 4.1 PlatformServices 概述

`PlatformServices` 是平台级服务的注册与查询中心，作为核心与外部服务之间的桥梁，允许宿主应用注册服务供插件使用。核心模块不感知具体服务类型，保持纯粹的插件运行时职责。
...
- 宿主应用注册 `PluginService`、`ConfigService` 等管理服务
- 插件通过类型安全的方式获取所需服务

### 4.3 服务注册示例

宿主应用通过 Spring Bean 暴露服务，平台自动管理：
...
public class PluginService {
    ...
    public PluginService(PluginManager pluginManager, PluginFacade pluginFacade) {
...
继承 `AbstractPlugin` 的插件可直接通过便捷方法访问上下文：
...
public class MyPlugin extends AbstractPlugin {
    ...
    protected void start() throws Exception {
        // 访问工作空间（首次访问时自动创建目录）
        Path configFile = workspace().config().resolve("settings.json");

        // 注册扩展实现
        extensions().register(StorageProvider.class, new MyStorageProvider());

        // 发布事件
        events().publish(new MyPluginStartedEvent());
    }
...
插件通过 `context.platform().config()` 访问配置管理器。详见 [配置中心](配置中心.md)。
