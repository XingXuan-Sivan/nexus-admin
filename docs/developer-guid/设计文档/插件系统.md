...
| 模块 | 职责 | 说明 |
|------|------|------|
| PluginManager | 插件生命周期管理 | 协调发现、解析、加载、启动、停止、卸载全流程，通过 Facade 间接访问底层组件 |
...
| PluginRuntime | 插件运行时 | 以 Builder 模式装配插件子系统全部默认实例，不依赖外部 DI 容器 |
| PluginFacade | 插件组件门面 | 聚合插件发现、解析、加载、注册等子组件，提供统一查询与组件创建入口 |
| CoreConfig | 核心运行时配置 | 封装 RuntimeMode 和 pluginsDataRoot，避免构造函数参数膨胀 |
...
**构造方式**：通过 `CoreConfig` + 4 个 `Facade` 构造，避免底层组件引用直接暴露。

```java
// 构造示例
PluginManager pm = new DefaultPluginManager(
    coreConfig,       // CoreConfig: RuntimeMode + pluginsDataRoot
    pluginFacade,     // PluginFacade: 插件注册中心、加载器、解析器等
    extensionFacade,  // ExtensionFacade: 扩展注册中心
    configFacade,     // ConfigFacade: 配置管理器
    eventBusFacade    // EventBusFacade: 事件总线
);
```
