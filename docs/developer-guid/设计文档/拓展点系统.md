...
- **事件驱动**：扩展实现的注册/注销通过 EventBus 发布 ExtensionChangedEvent，支持消费方动态感知变更
...
在 `register()`/`unregister()`/`unregisterByPluginId()`/`clear()`/`clearAll()` 操作完成后，通过构造函数注入的 EventBus 发布 `ExtensionChangedEvent`。EventBus 可为 null（向后兼容），此时不发布事件。

```java
public class DefaultExtensionRegistry implements ExtensionRegistry {

    private final EventBus eventBus;  // 事件总线，可为 null（向后兼容）
...
#### 2.2.5 ExtensionConsumer（扩展点消费者模板）

扩展点通用消费者模板，位于 core 模块，提供双重缓存和事件驱动失效机制。消费方应通过此模板获取扩展实现，而非直接调用 ExtensionRegistry。

```java
public class ExtensionConsumer<T extends ExtensionPoint> {

    private final Class<T> pointType;              // 扩展点接口类型
    private final ExtensionRegistry registry;       // 扩展注册中心
    private volatile List<T> cachedAll;             // 缓存的所有实现列表（按优先级降序）
    private volatile T cachedTop;                   // 缓存的最高优先级实现

    /**
     * 构造时即订阅扩展变更事件，当关联扩展点发生注册/注销/清空时自动失效缓存。
     */
...
- **双重缓存**：`cachedTop` 缓存最高优先级实现，`cachedAll` 缓存完整实现列表，避免频繁查询注册中心
- **事件驱动失效**：构造时订阅 `ExtensionChangedEvent`，当关联扩展点发生变更时自动清除缓存，下次访问时重新解析
- **线程安全**：使用 `volatile` + 同步块的双重检查锁定模式

------

#### 2.2.6 ExtensionWrapper（扩展包装器）
...
| DefaultExtensionRegistry | 扩展注册中心默认实现，操作完成后发布 ExtensionChangedEvent | core |
| ExtensionConsumer | 扩展点通用消费者模板，提供双重缓存与事件驱动失效机制 | core |
...
| ExtensionRuntime | 扩展注册中心运行时，以 Builder 模式装配 ExtensionRegistry 默认实例 | core |
| ExtensionFacade | 扩展注册中心门面，聚合 ExtensionRegistry 提供统一的注册、查询与注销入口 | core |
...
扩展注册中心的装配由 `ExtensionRuntime` 负责，支持 Builder 模式选择性覆盖：
...
引入 `nexus-admin-api` 模块后，`ExtensionAutoConfig` 自动将 ExtensionRuntime 装配的组件桥接到 Spring 容器，可通过 `@Autowired` 注入 ExtensionRegistry 或 ExtensionFacade。

```java
@Configuration
public class BootstrapConfig {

    // 覆盖默认 ExtensionRuntime（可选）
    @Bean
    public ExtensionRuntime extensionRuntime(EventBus eventBus) {
        return ExtensionRuntime.builder()
            .eventBus(eventBus)
